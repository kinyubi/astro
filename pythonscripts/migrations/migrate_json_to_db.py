...
def main():
    ...
    updated = skipped_redirect = skipped_no_row = redirects_added = 0

    # First pass: register redirect entries (e.g. M1 -> NGC1952) as CatalogIDs
    for dso_key, entry in data.items():
        if not is_redirect(entry):
            continue
        target_key = entry["See"]
        cur.execute("SELECT DSOKey FROM Objects WHERE DSOKey = ?", (target_key,))
        if not cur.fetchone():
            print(f"  SKIP redirect {dso_key} -> {target_key} (target not in Objects)")
            skipped_redirect += 1
            continue
        cur.execute("""
        ...
        """, (dso_key, target_key))
        if cur.rowcount:
            print(f"  ALIAS {dso_key:10s} -> {target_key}")
            redirects_added += 1
        else:
            skipped_redirect += 1

    # Second pass: update object fields from full entries
    for dso_key, entry in data.items():
    ...
    print(f"  Redirects already present : {skipped_redirect}")
    print(f"  No matching DB row        : {skipped_no_row}")
    print(f"  OtherNames aliases added  : {other_names_added}")
