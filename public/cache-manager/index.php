 ...
 * Route: /cache-manager
 *
 * View and manage cached DSO reports stored in vis/cache/
 */

header('Content-Type: text/html; charset=utf-8');

// Cache lives in public/vis/cache/
$cacheDir = dirname(__DIR__) . DIRECTORY_SEPARATOR . 'vis' . DIRECTORY_SEPARATOR . 'cache';
...
if ($action === 'delete' && $file) {
    ...
    if (file_exists($filePath) && strpos($file, 'dso_report_') === 0) {
        $message = unlink($filePath)
            ? "<div class='success'>✓ Deleted: $file</div>"
...
if (is_dir($cacheDir)) {
    ...
    usort($cacheFiles, fn($a, $b) => strcmp($b['date'], $a['date']));
}
...
<head>
    ...
    <style>
        ...
        .info    { background: #1a1f3a; padding: 15px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #4a9eff; }
        .success { background: #1a3a1f; padding: 15px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #7ec8a3; color: #7ec8a3; }
        .error   { background: #3a1a1f; padding: 15px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #ff6b6b; color: #ff6b6b; }
        .actions { margin: 20px 0; display: flex; gap: 10px; flex-wrap: wrap; }
        .btn { padding: 10px 20px; background: #2a3f5f; color: #4a9eff; text-decoration: none; border-radius: 4px; border: 1px solid #4a9eff; cursor: pointer; display: inline-block; transition: background 0.2s; }
        ...
        th { background: #2a3f5f; color: #4a9eff; padding: 12px; text-align: left; font-weight: 600; }
        td { padding: 10px 12px; border-bottom: 1px solid #2a3f5f; }
        tr:hover { background: #243447; }
        .empty { text-align: center; padding: 40px; color: #7a7a7a; }
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <a href="?action=delete&file=<?php echo urlencode($cf['filename']); ?>"
                               ...
                               onclick="return confirm('Delete cache for <?php echo htmlspecialchars($cf['date']); ?>?')">Delete</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
