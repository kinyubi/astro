...
// Paths — pythonscripts is two levels up from public/profiles/
$pythonDir = dirname(dirname(__DIR__)) . DIRECTORY_SEPARATOR . 'pythonscripts';
    ...
    if ($action === 'geocode') {
        ...
        if (empty($location)) {
            ...
            if ($result && isset($result['success']) && $result['success']) {
                ...
                $message = "❌ Geocoding failed: " . htmlspecialchars($result['error'] ?? "Unknown error");
                $messageType = 'error';
            }
        }
        ...
        if (!$validation['valid']) {
            ...
            if ($result && isset($result['success']) && $result['success']) {
                ...
                $message = "❌ Failed to create profile: " . htmlspecialchars($result['error'] ?? "Unknown error occurred");
                $messageType = 'error';
            }
        }
        ...
        if (empty($profileName)) {
            ...
            if ($result && isset($result['success']) && $result['success']) {
                ...
                $message = "❌ Failed to update profile: " . htmlspecialchars($result['error'] ?? "Unknown error occurred");
                $messageType = 'error';
            }
        }
        ...
        if (empty($profileName)) {
            ...
            if ($result && isset($result['success']) && $result['success']) {
                ...
                $message = "❌ Failed to delete profile: " . htmlspecialchars($result['error'] ?? "Unknown error occurred");
                $messageType = 'error';
            }
        }
    }
...
<head>
    ...
    <style>
        body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 1200px; margin: 0 auto; padding: 20px; background: #0a0e27; color: #e0e0e0; }
        h1, h2 { color: #4a9eff; border-bottom: 2px solid #4a9eff; padding-bottom: 10px; }
        .back-link { display: inline-block; margin-bottom: 20px; color: #4a9eff; text-decoration: none; }
        .back-link:hover { text-decoration: underline; }
        .message { padding: 15px; border-radius: 8px; margin: 20px 0; border-left: 4px solid; }
        .message.success { background: #1a3a1f; border-color: #7ec8a3; color: #7ec8a3; }
        .message.error   { background: #3a1a1f; border-color: #ff6b6b; color: #ff6b6b; }
        .message.info    { background: #1a1f3a; border-color: #4a9eff; color: #4a9eff; }
        .section { background: #1a1f3a; padding: 20px; border-radius: 8px; margin: 20px 0; }
        .section.edit-mode { border: 2px solid #ffd700; }
        .form-group { margin: 15px 0; }
        label { display: block; color: #4a9eff; margin-bottom: 5px; font-weight: 600; }
        input[type="text"], input[type="number"], select { width: 100%; max-width: 400px; padding: 10px; background: #2a3f5f; border: 1px solid #4a9eff; border-radius: 4px; color: #e0e0e0; font-size: 14px; }
        input[type="text"]:focus, input[type="number"]:focus, select:focus { outline: none; border-color: #7ec8a3; }
        input[type="text"].error, input[type="number"].error { border-color: #ff6b6b; }
        input[type="text"]:read-only { background: #1a2a3f; color: #888; }
        .btn { padding: 10px 20px; background: #2a3f5f; color: #4a9eff; text-decoration: none; border-radius: 4px; border: 1px solid #4a9eff; cursor: pointer; display: inline-block; transition: background 0.2s; font-size: 14px; }
        .btn:hover { background: #3a4f6f; }
        .btn-primary { background: #4a9eff; color: #0a0e27; border-color: #4a9eff; }
        .btn-primary:hover { background: #5aaeff; }
        .btn-warning { background: #5a4a00; border-color: #ffd700; color: #ffd700; }
        .btn-warning:hover { background: #6a5a10; }
        .btn-danger { background: #4a2020; border-color: #ff6b6b; color: #ff6b6b; }
        .btn-danger:hover { background: #5a3030; }
        .btn-secondary { background: #2a3f5f; border-color: #888; color: #888; }
        .btn-secondary:hover { background: #3a4f6f; color: #aaa; }
        .profiles-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 15px; margin-top: 20px; }
        .profile-card { background: #2a3f5f; padding: 15px; border-radius: 8px; border: 1px solid #4a9eff; }
        .profile-card.is-default { border-color: #7ec8a3; }
        .profile-card h3 { margin-top: 0; color: #4a9eff; border-bottom: 1px solid #4a9eff; padding-bottom: 8px; }
        .profile-card.is-default h3 { color: #7ec8a3; border-color: #7ec8a3; }
        .profile-card p { margin: 8px 0; font-size: 0.9em; }
        .profile-card .actions { margin-top: 15px; display: flex; gap: 10px; flex-wrap: wrap; }
        .helper-text { font-size: 0.85em; color: #b8c5d6; margin-top: 5px; }
        .helper-text.error { color: #ff6b6b; }
        .two-col { display: grid; grid-template-columns: 1fr 1fr; gap: 15px; }
        .validation-rules { background: #0a0e27; padding: 10px 15px; border-radius: 4px; margin: 10px 0; font-size: 0.85em; }
        .validation-rules ul { margin: 5px 0; padding-left: 20px; }
        .validation-rules li { margin: 3px 0; }
        .validation-rules li.valid { color: #7ec8a3; }
        .validation-rules li.invalid { color: #ff6b6b; }
        @media (max-width: 768px) { .two-col { grid-template-columns: 1fr; } .profiles-grid { grid-template-columns: 1fr; } }
    </style>
</head>
<body>
        ...
        <form method="POST" id="createForm">
            ...
            <div class="form-group">
                ...
                <input type="text" id="location" name="location" required
                       placeholder="e.g., Star, Idaho or New York, NY">
                <p class="helper-text">City, State or City, Country format works best.</p>
            </div>
            <div class="two-col">
                <div class="form-group">
                    <label for="min_altitude">Minimum Altitude (degrees)</label>
                    <input type="number" id="min_altitude" name="min_altitude" value="18" min="0" max="90" step="0.1">
                    <p class="helper-text">Objects below this altitude won't be shown</p>
                </div>
                <div class="form-group">
                    <label for="az_min">Azimuth Min (degrees)</label>
                    <input type="number" id="az_min" name="az_min" value="10" min="0" max="360" step="0.1">
                    <p class="helper-text">Minimum azimuth (0° = North)</p>
                </div>
            </div>
            <div class="form-group">
                <label for="az_max">Azimuth Max (degrees)</label>
                <input type="number" id="az_max" name="az_max" value="165" min="0" max="360" step="0.1">
                <p class="helper-text">Maximum azimuth (180° = South)</p>
            </div>
            ...
            <div class="profiles-grid">
                <?php foreach ($profiles as $profile): ?>
                    <div class="profile-card <?php echo $profile['name'] === 'default' ? 'is-default' : ''; ?>">
                        ...
                        <div class="actions">
                            <a href="/vis?profile=<?php echo urlencode($profile['name']); ?>" class="btn">🔭 Use</a>
                            <a href="/profiles?edit=<?php echo urlencode($profile['name']); ?>" class="btn btn-warning">✏️ Edit</a>
                            <?php if ($profile['name'] !== 'default'): ?>
    ...
    <script>
        function validateProfileNameLive(input) {
            ...
            for (const [ruleId, isValid] of Object.entries(rules)) {
                ...
                if (el) { el.classList.remove('valid', 'invalid'); if (name !== '') el.classList.add(isValid ? 'valid' : 'invalid'); }
            }
            input.classList.toggle('error', !overallValid && name !== '');
            const btn = document.getElementById('createBtn');
            if (btn) btn.disabled = !overallValid && name !== '';
            return overallValid;
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (createForm && profileNameInput) {
                createForm.addEventListener('submit', function(e) {
                    ...
                    if (!name) { alert('Profile name is required'); e.preventDefault(); return; }
                    if (!/^[a-z0-9_]+$/.test(name)) { alert('Profile name must contain only lowercase letters, numbers, and underscores.'); e.preventDefault(); return; }
                });
                if (profileNameInput.value) validateProfileNameLive(profileNameInput);
            }
        });
    </script>
</body>
